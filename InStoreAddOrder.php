...
(
	...
	(
		...
		(
            ...
            'State' => 'Initial',
			/* 这三项的设置与物流计划有关  */
			'Warehouse' => 'AU',	//处理仓库代码，枚举类型,取值范围为US（美国）、AU（澳大利亚）、UK（英国）、MA（曼城）
			'Location' => 'GZ',	//出口易处理点，可选值有三个，分别是GZ（广州）、SZ（深圳）、SH（上海）
			'ShippingMethod' =>	'DHL',	//物流方式,可选值为AIR(空运)/SEA(海运)/EMS(中国邮政速递物流)/DHL(中外运敦豪国际航空快递有限公司)/UPS(美国邮政)		
			/* 这三项的设置与物流计划有关 */
...
$result = $soap_client->InStoreAddOrder($request);		//用户通过调用该方法添加入库单，添加成功返回入库订单号
echo "<pre>";
print_r($result);
echo "</pre>";
